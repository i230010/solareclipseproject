...
def penumbra_distance(
    ...
    ----------
    t : float
        Time parameter (TT units in hours).
...
def startendtime(
    ...
    te_start: float = fsolve(
        lambda t: penumbra_distance(t, x_coeffs, y_coeffs, l_coeffs), -4
    )
    ...
    start_time: float | None = float(te_start[0]) if te_start.size > 0 else None
    end_time: float | None = float(te_end[0]) if te_end.size > 0 else None
